...
import { Animated, Image, ScrollView, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, View, Alert, ActivityIndicator } from 'react-native';
...
export default function DetailScreen() {
    ...
    const handleBuyPressIn = () => Animated.spring(buyScale, { toValue: 0.95, useNativeDriver: true }).start();
    const handleBuyPressOut = () => Animated.spring(buyScale, { toValue: 1, friction: 3, useNativeDriver: true }).start();

    const handleSimpanKomentar = () => {
        if (!komentar.trim()) { Alert.alert('Oops!', 'Komentar tidak boleh kosong.'); return; }
        setDaftarKomentar(prev => [{ id: Date.now(), teks: komentar.trim(), waktu: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }, ...prev]);
        setKomentar('');
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView>
                ...
                <Animated.View style={[styles.imageContainer, { opacity: imageOpacity }]}>
                    ...
                    <TouchableOpacity style={styles.bookmarkIcon} onPress={handleBookmarkPress} activeOpacity={1}>
                        <Animated.View style={{ transform: [{ scale: bookmarkScale }] }}>
                            <Ionicons name={isBookmarked ? 'bookmark' : 'bookmark-outline'} size={24} color={colors.secondary} />
                        </Animated.View>
                    </TouchableOpacity>
                </Animated.View>
                ...
                <Animated.View style={[styles.content, { opacity: contentOpacity, transform: [{ translateY: contentSlide }] }]}>
                    ...
                    {product.brand && (<><Text style={styles.sectionTitle}>Brand</Text><Text style={styles.brand}>{product.brand}</Text></>)}

                    <Animated.View style={{ transform: [{ scale: buyScale }] }}>
                        <TouchableOpacity style={styles.buyButton} onPressIn={handleBuyPressIn} onPressOut={handleBuyPressOut} activeOpacity={1}>
                            <Ionicons name="cart-outline" size={20} color={colors.primary} />
                            <Text style={styles.buyButtonText}>Lihat Link Pembelian</Text>
                        </TouchableOpacity>
                    </Animated.View>

                    {/* TOMBOL DELETE SUPABASE */}
                    <TouchableOpacity style={styles.deleteButton} onPress={handleDelete} disabled={deleteLoading}>
                        {deleteLoading ? <ActivityIndicator color="#fff" /> : (
                            <><Ionicons name="trash-outline" size={18} color="#fff" /><Text style={styles.deleteButtonText}>Hapus dari Supabase</Text></>
                        )}
                    </TouchableOpacity>

                    <View style={styles.divider} />
                    <Text style={styles.sectionTitle}>Komentar Anda:</Text>
                    <TextInput style={styles.commentInput} placeholder={`Tulis komentar...`} placeholderTextColor="#666" value={komentar} onChangeText={setKomentar} multiline numberOfLines={4} />
...
const styles = StyleSheet.create({
    ...
    backIcon: { position: 'absolute', top: 16, left: 16, backgroundColor: 'rgba(0,0,0,0.5)', padding: 8, borderRadius: 30 },
    bookmarkIcon: { position: 'absolute', top: 16, right: 16, backgroundColor: 'rgba(0,0,0,0.5)', padding: 8, borderRadius: 30 },
    content: { padding: 20 },
    ...
    buyButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: colors.secondary, paddingVertical: 14, borderRadius: 12, marginTop: 24, marginBottom: 10 },
    buyButtonText: { color: colors.primary, fontSize: 16, fontWeight: 'bold' },
    deleteButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#cc2222', paddingVertical: 13, borderRadius: 12, marginBottom: 10 },
    deleteButtonText: { color: '#fff', fontSize: 15, fontWeight: 'bold' },
    commentInput: { backgroundColor: '#2a2a2a', borderRadius: 12, padding: 14, color: '#fff', fontSize: 14, borderWidth: 1, borderColor: colors.secondary + '40', textAlignVertical: 'top', minHeight: 100, marginBottom: 12 },
    commentButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: colors.secondary, paddingVertical: 13, borderRadius: 12, marginBottom: 16 },
    commentButtonText: { color: colors.primary, fontSize: 15, fontWeight: 'bold' },
    komentarList: { marginTop: 8, marginBottom: 30 },
    komentarItem: { backgroundColor: '#2a2a2a', borderRadius: 12, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: colors.secondary + '20' },
    ...
    backButton: { marginTop: 20, backgroundColor: colors.secondary, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 25 },
